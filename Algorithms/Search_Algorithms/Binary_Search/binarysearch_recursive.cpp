...
long long int binarySearch(long long int arr[], long long int l, long long int r, long long int x)
{
        if (r >= l)
        {
                ...
                if (arr[mid] == x)  return mid;/*if found return the index*/

                if (arr[mid] > x) return binarySearch(arr, l, mid-1, x);/*search in left subarray*/
...
int main()
{
        ...
        for (i=0;i<n;i++)/*input sorted array of size n*/
        {
            scanf ("%lld",&inarr[i]);
        }
        ans=binarySearch(inarr,0,n-1,5);
        if (ans!=-1)/*if non-negative index exists*/
        {
            printf ("Exists\n");
        }
